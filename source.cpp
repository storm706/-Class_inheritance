...
int main()
{
	...
	flc.Flight();
	flc.FlightDrive();
	flc.Car::Use();// обращение с помощью пространства имен
	flc.Plane::Use();
	
	cout << "\tCalling a specific similar method from the base class creates a new object" << endl;
	((Car)flc).Use();//обращение с помощью приведения типа.вызов конкретного схожего метода из базового класса. создает новый объект
	((Plane)flc).Use();
